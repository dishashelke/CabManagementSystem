          ...
          <div id="middle">
             ...
             <%
              ...
              if(ss.equalsIgnoreCase("N"))
              {
                    ...
                    {
                        System.out.println("Emp are ="+emp_name[i]);
                        String sql3="select emp_gender from EmployeeMaster where emp_name='"+emp_name[i]+"' ";
                        ResultSet rst3=stmt.executeQuery(sql3);
                        rst3.next();
                        String empgender=rst3.getString(1);
                    ...
                    if(flag==1)
                      {
                          System.out.println("Iniiifff1111");
                          if(guard_name.equalsIgnoreCase("SELECT"))
                            {

                            System.out.println("Iniiifff222222");
                                %>
                                The shift is night there is a lady in a cab.Please select a guard.<br/>
                                <a href="DropAlloc.jsp"> BACK..</a>
                                <%
                            }
                          else
                         {
                                    if(emp_name.length>3)
                                    {
                                        %>
                                        Please select at most three employee's.<br/>
                                        <a href="DropAlloc.jsp">BACK..</a>
                                        <%
                                    }
                                    else
                                         ...
                                         while(i<emp_name.length)
                                            {
                                                ...
                                                String empgen=rst1.getString(7);

                                                String sql6="select * from GuardMaster where guard_name='"+guard_name+"'";
                                                ResultSet rst6 = stmt.executeQuery(sql6);
                                                rst6.next();
                                                String guard_id=rst6.getString(1);
                                                if(empgen.equalsIgnoreCase("F"))
                                                {
                                                    String sql="insert into Execution values('"+cab_id+"','"+route_id+"','"+driver_id+"','"+emp_id+"','"+guard_id+"','a101',current_timestamp,'','0')";
