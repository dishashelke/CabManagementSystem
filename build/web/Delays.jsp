...
Notifications
          ...
          <form action="DelayMessage.jsp" method="POST">
          <pre>

       <h2> Inform About Delay</h2>
            ...
            String sql="select delayed_by,cab_id from Exception where description='Delay' && exception_status = 0 && (select date(exception_time)) = CURDATE()";
            rst = stmt.executeQuery(sql);
            int cnt=1;
            if(!rst.next())
                {
                %>
                <%--out.println("No Delay messages from Anyone");--%>
                 <h3><center>No Delay Notification From Anyone..........</center></h3>
               <%
                }
            else
            {
            String sql1="select delayed_by,cab_id from Exception where description='Delay' && exception_status = 0 && (select date(exception_time)) = current_date";
            rst1 = stmt1.executeQuery(sql1);

            while(rst1.next())
            {

                delayedby = rst1.getString(1);
                cab_id = rst1.getString(2);
                //driver_license_no=rst1.getString(3);
                String sql2 = "select route_id from Execution where cab_id='"+cab_id+"' && (select date(alloc_time)) = CURDATE() && execution_status='0'";
                System.out.println(sql);
                ResultSet rst2 = stmt.executeQuery(sql2);
                while(rst2.next())
                {
                route_id = rst2.getString(1);
                }
                System.out.println("Cab id    "+cab_id);

                System.out.println("Delayed by  "+delayedby);

                 %>
  <%=cnt%>)
                 <h3> Cab Id :         <input type="text" name="cab_id" value="<%=cab_id%>" readonly="true"</h3>
                 <h3> Route Id  :     <input type="text" name="route_id" value="<%=route_id%>" readonly="true"</h3>
                 <h3> Delayed by :     <input type="text" name="delayedby" value="<%=delayedby%>" readonly="true"</h3>
                 <%
                 cnt++;
             }
                %>
            ...
            <input type="Submit" name="submit" value="Send Message to Employee ">
            <%
            }
            %>
          </pre>
