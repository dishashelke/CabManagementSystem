...
Notifications
          ...
          <div id="middle">
              <form action="actionFeedback.jsp" method="Post" onsubmit="return message()">
              <h3>Feedback Of Driver</h3>
              <%-- <table border="1">
                                    <tr>
                                     <th></th>
                                    <th>Driver ID</th>
          ...
          //  String cab_id = request.getParameter("cab_id");
            ...
            if(!rst.next())
                    {
                        out.println("No Feedback about any Driver............");
                    }
            else
            {
                %>
            ...
            while(rst2.next())
            {
                ...
                String sql="select driver_license_no from DriverMaster";
                rst1 = stmt1.executeQuery(sql);
                while(rst1.next())
                {
                     driver_id = rst1.getString(1);
                     if(driver_id.equalsIgnoreCase(feedback_about))
                          {
                          %>

                                    <tr>
                                        <td><input type="radio" name="user" value="<%=driver_id%>,<%=description%>"></td>
                                        <td><%=driver_id%></td>
